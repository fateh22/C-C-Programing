...
Tree* lookUp(struct Tree *node, char key) {
  ...
  if(node->data == key) return node;
    ...
    if(node->data < key)
    return lookUp(node->right, key) ; else
...
void reverseOrderPrint(struct Tree *node) {
 ...
 Tree *lowestCommonAncestor(Tree *node, Tree *p, Tree *q) {
   Tree *left, *right;
   if(node == NULL) return NULL;
   if(node->left == p || node->left == q || node->right == p || node->right == q) return node;
...
/*
  ...
  right hand pointers are swapped at every node
*/
void mirror(Tree *r) {
  if(r == NULL) return;
  Tree *tmp;
...
int main(int argc, char **argv) {
  ...
  if(found) {
    cout << "Min value of subtree " << ch
      << " as a root is " << minTree(found)->data << endl;
    cout << "Max value of subtree " << ch
      << " as a root is " << maxTree(found)->data << endl;
  }
  ...
  if(found) {
    ...
    if(succ)
      cout << "In Order Successor of " << ch
        << " is " << succesorInOrder(found)->data << endl;
    else
  ...
  if(found) {
    ...
    if(pred)
      cout << "In Order Predecessor of " << ch
        << " is " << predecessorInOrder(found)->data << endl;
    else
  ...
  if(ancestor)
    cout << "The lowest common ancestor of " << ch1
      << " and " << ch2 << " is " << ancestor->data << endl;
